...
- **Data directories must be `0o700`** — the `~/.cmp/<user>/` directory contains plaintext message DB, WAL/SHM sidecar files, and key material. Restrict the parent directory rather than individual files to cover all current and future files in one shot.
