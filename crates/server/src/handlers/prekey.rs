...
async fn store_signed_prekey_rotation(
    ...
    let accepted =
        match db::prekeys::rotate_signed_prekey(&state.db, user_id, key_id, public_key, signature)
            .await
        {
            Ok(db::prekeys::SignedPreKeyRotationResult::Accepted) => true,
            Ok(
                db::prekeys::SignedPreKeyRotationResult::InvalidSequence
                | db::prekeys::SignedPreKeyRotationResult::Conflict,
            ) => false,
            Err(error) => {
